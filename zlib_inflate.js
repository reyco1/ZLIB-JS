(function (GLOBAL) {
  ...
  var ZLIB = {}; // ZLIB namespace initialization

// common definitions
  if ("undefined" === typeof ZLIB.common_initialized) {
    ZLIB.Z_NO_FLUSH = 0;
    ...
    ZLIB.z_stream = function () {
      ...
      /* next input byte */
      this.avail_in = 0;
      /* number of bytes available in input_data */
      this.total_in = 0;
      ...
      this.adler = 0;
      /* TODO: adler32 value of the uncompressed data */
      ...
      this.error = 0;
      /* error code */
      this.checksum_function = null;
      /* crc32(for gzip) or adler32(for zlib) */
    };
    ...
    ZLIB.gz_header = function () {
      ...
      /* operating system */
      this.extra = null;
      /* extra field string or null if none */
      this.extra_len = 0;
      /* this.extra.length (only when reading header) */
      this.extra_max = 0;
      /* space at extra (only when reading header) */
      this.name = null;
      /* file name string or null if none */
      this.name_max = 0;
...
// inflate_mode
    ...
    var inflate_table_lbase = [3, 4, 5, 6, 7, 8, 9, 10, 11, 13, 15, 17, 19, 23, 27, 31, 35, 43, 51, 59, 67, 83, 99, 115, 131, 163, 195, 227, 258, 0, 0];

    /* Length codes 257..285 extra */
    var inflate_table_lext = [16, 16, 16, 16, 16, 16, 16, 16, 17, 17, 17, 17, 18, 18, 18, 18, 19, 19, 19, 19, 20, 20, 20, 20, 21, 21, 21, 21, 16, 203, 69];

    /* Distance codes 0..29 base */
    var inflate_table_dbase = [1, 2, 3, 4, 5, 7, 9, 13, 17, 25, 33, 49, 65, 97, 129, 193, 257, 385, 513, 769, 1025, 1537, 2049, 3073, 4097, 6145, 8193, 12289, 16385, 24577, 0, 0];

    /* Distance codes 0..29 extra */
    var inflate_table_dext = [16, 16, 16, 16, 17, 17, 18, 18, 19, 19, 20, 20, 21, 21, 22, 22, 23, 23, 24, 24, 25, 25, 26, 26, 27, 27, 28, 28, 29, 29, 64, 64];
...
//    ZLIB.inflate_copyright =' inflate 1.2.6 Copyright 1995-2012 Mark Adler ';
    ...
    function inflate_fast(strm,
      ...
      var state;
      var input_data;
      /* local strm->input_data */
      var next_in;
      /* zlib.js: index of input_data */
      var last;
      /* while next_in < last, enough input available */
      var out;
      /* local strm.next_out */
      var beg;
...
//#endif
      ...
      var from_window_offset = -1;
      /* index of window[] */
      var from_out_offset = -1;
      /* index of next_out[] */
...
//NOTREACHED }
    ...
    function inflate_state() {
      ...
      /* valid bytes in the window */
      this.wnext = 0;
      /* window write index (TODO remove) */
      this.window = null;
      ...
      this.lens = new_array(320);
      /* temporary storage for code lengths */
      this.work = new_array(288);
      /* work area for code table building */
      this.codes = new Array(ENOUGH);
...
// Usage: strm = ZLIB.inflateReset(z_stream [, windowBits]);
    ...
    ZLIB.inflateReset = function (strm, windowBits) {
      ...
      if (typeof windowBits === "undefined")
        windowBits = DEF_WBITS.ZLIB_ENCODING_DEFLATE;

      /* extract wrap request from windowBits parameter */
      if (windowBits < 0) {
        wrap = 0;
        windowBits = DEF_WBITS.ZLIB_ENCODING_RAW; // is === -windowBits;
      }
      else {
        wrap = (windowBits >>> 4) + 1;
        if (windowBits < 48)
...
// #ifdef GUNZIP
    ...
    function LOAD(strm, s) {
      s.strm = strm;
      /* z_stream */
      s.left = strm.avail_out;
      /* available output */
      s.next = strm.next_in;
      /* next input */
      s.have = strm.avail_in;
      /* available input */
      s.hold = strm.state.hold;
      /* bit buffer */
      s.bits = strm.state.bits;
      /* bits in bit buffer */
      return s;
    }
    ...
    var inflate_order = [16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15];
...
//#endif
  ...
  /**
   *
   * @param {string} data - a base64 encoded of a compressed result
   * of PHP's zlib_encode("raw string",ZLIB_ENCODING_DEFLATE) function.
   * @returns {string}
   */
  ZLIB.decode = function (data) {
    return ZLIB.inflateInit().inflate(atob(data));
  };
